...
export const IDENTITY_URL = `${DISCOGS_BASE_URL}/oauth/identity`;
...
export const ACCESS_TOKEN_CONFIG = (token, secret, verifier) => {
  return {
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      Authorization: `OAuth oauth_consumer_key="${CONSUMER_KEY}",oauth_nonce="${Date.now()}",oauth_token="${token}",oauth_signature="${CONSUMER_SECRET}&${secret}",oauth_signature_method="PLAINTEXT",oauth_timestamp="${Date.now()}",oauth_verifier="${verifier}"`,
      'User-Agent':
